...
def copyDirStruct(inPath: str, outPath: str, exclude=None):
    if exclude is None:
        exclude = []
...
def copyDirStructExtToList(root: str, ext='.epub') -> list:
    ...
    return [p for p in filelist if (not Path(p).stem.startswith('._')) and (Path(p).suffix == ext)]
...
def make_archive_threadsafe(zip_name: str, path: str):
    with zipfile.ZipFile(zip_name, 'w', zipfile.ZIP_DEFLATED) as zip_f:
        for root, dirs, files in os.walk(path):
            for file in files:
                zip_f.write(os.path.join(root, file), os.path.relpath(os.path.join(root, file), path))
